...
<p
  ...
  <v-card height="220px" width="300px" elevation="18" style="border-radius:20px;padding:16px;padding-top:40px">
  <v-row>
  <div style="float:left;width:40%">
  <img :src="book.bookcover" height="130px" width="90px"/>
  </div>
  <div style="float:left;width:50%">
  <div v-if="book.title.length < 20" style="text-align:left;font-size:18px" class="font-weight-bold">{{book.title}}</div>
  <div v-else style="text-align:left;font-size:18px" class="font-weight-bold">{{book.title.slice(0,20)}}...</div>
  <div style="text-align:left;font-size:16px" class="grey--text">{{book.author}}</div>
  <div style="margin-right:19px;width:1px">
  <v-rating dense  :value="parseFloat(book.rating)" width=1px color="#f6b911" size="14" />
  </div>
  <v-btn :to="`/audio/${book.slug}`" style="float:right;margin-bottom:20px" small fab class="pulsingButton" color="black"> <v-icon color="white">mdi-play</v-icon></v-btn>
...
<p
      ...
      >
        ...
        >
          ...
          <v-btn large fixed absolute bottom right style="margin-bottom:100px" fab color="black" class="pulsingButton" to="/publish"><v-icon color="white">mdi-plus</v-icon></v-btn>
      </v-card>
    </v-main>
    <BottomMenu />
  </v-app>
...
export default {
  ...
  computed:{
   ...
   filteredAudio(){
      ...
      const audioBooks = this.books.filter(book => book.filetype == "Audio")
      if (!searchBooks) return audioBooks;
